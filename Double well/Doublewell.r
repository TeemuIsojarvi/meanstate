# This program calculates the ground state wave function of a 1D double well system with an unusual method
...
for(j in c(1:nx)) {
if(abs(j*dx-3)>2) V[j] = as.complex(5)
else {
if(abs(j*dx-3)<0.5) V[j] = as.complex(2.5)
else V[j] = as.complex(0)
}
}
...
for(j in c(1:nx)) {
psi_ground[j] = as.complex(exp(-0.5*(j*dx-3)*(j*dx-3)))					 #Gaussian initial wavefunction
}
...
for(j in c(1:nx)) {
psi[j] = as.complex(exp(-0.5*(j*dx-3)*(j*dx-3)))					 #Gaussian initial wavefunction
}
